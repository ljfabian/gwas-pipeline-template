...
```bash
git clone https://github.com/ljfabian/gwas-pipeline-template.git
```

Enter the directory:
```bash
cd gwas-pipeline-template
```

This pipeline requires a config file. There is an example file for DUs. If external, you may need to edit the pipeline further. 
create your own config file and edit it accordingly for correct paths:
```bash
cp code/example_config.json code/00_config.json
vim code/00_conifg.json
```
